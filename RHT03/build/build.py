...
so_file_path = os.path.join(output_dir, 'RHT03.so')
if generator == 'Unix Makefiles':
    subprocess.check_call(['make', '-j', str(cpu_count())], cwd=output_dir)
    if platform == 'Darwin':
        shutil.copy(os.path.join(output_dir, 'libRHT03.dylib'), so_file_path)
    else:
        shutil.copy(os.path.join(output_dir, 'libRHT03.so'), so_file_path)
else:
    subprocess.check_call(
        ['msbuild.exe', 'RHT03.sln', '/m:{}'.format(cpu_count()), '/t:RHT03', '/p:Platform=x64'], cwd=output_dir
    )
    shutil.copy(os.path.join(output_dir, 'Debug', 'RHT03.dll'), so_file_path)

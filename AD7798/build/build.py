...
so_file_path = os.path.join(output_dir, 'AD7798.so')
if generator == 'Unix Makefiles':
    subprocess.check_call(['make', '-j', str(cpu_count())], cwd=output_dir)
    if platform == 'Darwin':
        shutil.copy(os.path.join(output_dir, 'libAD7798.dylib'), so_file_path)
    else:
        shutil.copy(os.path.join(output_dir, 'libAD7798.so'), so_file_path)
else:
    subprocess.check_call(
        ['msbuild.exe', 'AD7798.sln', '/m:{}'.format(cpu_count()), '/t:AD7798', '/p:Platform=x64'], cwd=output_dir
    )
    shutil.copy(os.path.join(output_dir, 'Debug', 'AD7798.dll'), so_file_path)

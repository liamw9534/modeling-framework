...
so_file_path = os.path.join(output_dir, 'LSM6DSL.so')
if generator == 'Unix Makefiles':
    subprocess.check_call(['make', '-j', str(cpu_count())], cwd=output_dir)
    if platform == 'Darwin':
        shutil.copy(os.path.join(output_dir, 'libLSM6DSL.dylib'), so_file_path)
    else:
        shutil.copy(os.path.join(output_dir, 'libLSM6DSL.so'), so_file_path)
else:
    subprocess.check_call(
        ['msbuild.exe', 'LSM6DSL.sln', '/m:{}'.format(cpu_count()), '/t:LSM6DSL', '/p:Platform=x64'], cwd=output_dir
    )
    shutil.copy(os.path.join(output_dir, 'Debug', 'LSM6DSL.dll'), so_file_path)
